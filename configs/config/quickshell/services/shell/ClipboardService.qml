...
Singleton {
    ...
    Component.onCompleted: {
        root.pinnedItems = StateService.get("clipboard.pinned", []);
    }
    ...
    function clearAll() {
        console.log("[Clipboard] Clearing unpinned entries");
        clearProc.running = true;
    }

    function togglePin(text) {
        let pinned = root.pinnedItems.slice();
        const index = pinned.indexOf(text);
        if (index === -1) {
            pinned.push(text);
            console.log("[Clipboard] Pinned item");
        } else {
            pinned.splice(index, 1);
            console.log("[Clipboard] Unpinned item");
        }
        root.pinnedItems = pinned;
        StateService.set("clipboard.pinned", pinned);
    }
    ...
    Process {
        id: clearProc
        property string scriptPath: Quickshell.env("HOME") + "/.config/quickshell/scripts/tools/clear-unpinned.py"
        command: ["python3", scriptPath]

        onExited: (code) => {
            if (code === 0) {
                console.log("[Clipboard] Unpinned entries cleared");
                root.refresh();
            } else {
