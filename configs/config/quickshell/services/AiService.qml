...
import qs.config
import qs.services
import "../modules/aiChat/OrbitMessageParts.js" as OrbitMessageParts
...
Singleton {
    ...
    function _recomputeContextTokens() {
        ...
        for (const msg of messages)
            total += _estimateTokensForText(msg.content || "");
        for (const item of pendingAttachments)
            total += _estimateTokensForText(item.preview || "");
        approxContextTokens = total;
    }
    ...
    function _appendOrbitMessage(content, messageType, extra) {
        ...
        const next = messages.concat([Object.assign({
            role: "assistant",
    ...
    function _buildHistory(userText, attachments) {
        ...
        for (let i = updated.length - 1; i >= 0; --i) {
            const msg = updated[i];
            const cost = (msg.content || "").length + 48;
    ...
    function copilotStartAuth() {
        ...
        authProc.command = [
            "python3",
            Qt.resolvedUrl("../scripts/copilot_auth.py").toString().replace("file://", ""),
            "device_code"
        ];
    ...
    Component.onCompleted: {
        ...
        _recomputeContextTokens();
        if (storesReady && needsSetup)
            openSettings();
        else if (storesReady && !isFetchingModels)
            fetchModels();
    }
    ...
    Process {
        id: chatProc
        command: ["python3", Qt.resolvedUrl("../scripts/ai_chat.py").toString().replace("file://", "")]
        stdinEnabled: true
        running: false
