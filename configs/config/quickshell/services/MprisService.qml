...
Singleton {
    ...
    readonly property string title: {
        const t = activePlayer?.metadata?.["xesam:title"] ?? "";
        return t !== "" ? t : (activePlayer?.identity ?? "Unknown");
    }
    readonly property string artist: {
        const a = activePlayer?.metadata?.["xesam:artist"];
        if (!a)
            return "";
        // xesam:artist is an array in the MPRIS spec
        if (Array.isArray(a))
            return a.join(", ");
        return String(a);
    }
