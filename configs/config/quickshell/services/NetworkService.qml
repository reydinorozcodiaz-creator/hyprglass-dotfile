...
Singleton {
    ...
    readonly property string ethernetStatusText: ethernetConnected ? (ethernetConnection || "Connected") : "Disconnected"
    ...
    Process {
        id: getNetworksProc
        command: ["nmcli", "-t", "-e", "yes", "-f", "IN-USE,SIGNAL,SSID,SECURITY,BSSID,CHAN,RATE", "dev", "wifi", "list"]
        stdout: StdioCollector {
            onStreamFinished: {
                const trimmed = text.trim();
                const lines = trimmed.length > 0 ? trimmed.split("\n") : [];
