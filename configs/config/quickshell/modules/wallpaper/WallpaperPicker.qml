    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                // Next button — avanza la queue de AleatoryWall (anti-repetición)
                ActionButton {
                    icon: WallpaperManagerService.busy ? "󰔟" : "󰒭"
                    text: "Next"
                    enabled: !WallpaperManagerService.busy
                    opacity: WallpaperManagerService.busy ? 0.5 : 1.0
                    textColor: WallpaperManagerService.daemonRunning ? Config.accentColor : Config.subtextColor
                    hoverTextColor: Config.accentColor
                    onClicked: WallpaperService.nextWallpaper()
                }

                // Pywal toggle — integración de colores con AleatoryWall
                ActionButton {
                    icon: "󰉦"
                    text: "Pywal"
                    baseColor: WallpaperManagerService.pywalEnabled ? Config.accentColor : Config.surface1Color
                    hoverColor: WallpaperManagerService.pywalEnabled ? Config.accentColor : Config.surface2Color
                    textColor: WallpaperManagerService.pywalEnabled ? Config.textReverseColor : Config.subtextColor
                    hoverTextColor: WallpaperManagerService.pywalEnabled ? Config.textReverseColor : Config.textColor
                    onClicked: WallpaperManagerService.togglePywal()
                }
                ...
                // Random button — elige al azar de la lista visible (sin queue)
                ActionButton {
                    icon: "󰒝"
                    text: "Random"
